...
<div class="container mt-5">
    <div class="card shadow-sm">
        <div class="card-body">
            <h2 class="card-title text-center py-2">Reporte de Vehiculos</h2>
        ...
        try {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    doc.text('Reporte de Vehiculos', 105, 17, null, null, 'center');
                ...
                doc.text('Placa', 30, 37); // Ajusta la posición del texto Fecha
                doc.text('Stock', 60, 37); // Ajusta la posición del texto Cliente
                doc.text('Veh_Precio', 90, 37); // Ajusta la posición del texto Cantidad
                doc.text('Marca', 120, 37); // Ajusta la posición del texto Precio
                doc.text('Modelo', 150, 37); // Ajusta la posición del texto Subtotal
                doc.text('Color', 180, 37); // Ajusta la posición del texto Total
                ...
                data.forEach(row => {
                    ...
                    // doc.text(`${row.Veh_Placa}`, 12, yPosition);
                    doc.text(`${row.Veh_Placa}`, 30, yPosition);
                    doc.text(`${row.Veh_Stock}`, 60, yPosition);
                    doc.text(`${parseFloat(row.Veh_Precio).toFixed(2)}`, 90, yPosition);
                    doc.text(`${row.Mod_Descripcion}`, 120, yPosition);
                    doc.text(`${row.Mar_Descripcion}`, 150, yPosition);
                    doc.text(`${row.Veh_Color}`, 180, yPosition);
                    yPosition += 10;
